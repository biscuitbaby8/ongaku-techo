...
// --- Google AdSense 広告枠コンポーネント (SEO構造化) ---
const AdSlot = ({ type }) => (
...
export default function App() {
  const apiKey = ""; // Gemini API Key を設定してください
  ...
  const getAiMusic = async (term) => {
    ...
    try {
      const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({ contents: [{ parts: [{ text: `音楽用語「${term}」が象徴的な有名なクラシック曲を1つ挙げ、その理由を30文字程度で解説してください。` }] }] })
      });
      const data = await res.json();
      setAiAnalysis(data.candidates[0].content.parts[0].text);
    } catch (e) { setAiAnalysis("AI検索エラーが発生しました。"); }
    finally { setIsAiLoading(false); }
  };

  // --- メトロノームロジック (6/8対応・完全復元) ---
  ...
  const openCamera = async () => {
    ...
    } catch (err) { setScanError("カメラの起動に失敗しました。"); }
  };
  const captureAndScan = async () => {
    ...
    try {
      ...
      const data = await res.json(); const resText = data.candidates?.[0]?.content?.parts?.[0]?.text?.trim();
    ...
    } catch (e) { setScanError("解析エラー。光の加減を確認してください。"); } finally { setIsScanning(false); }
  };
  ...
  return (
    <div className={`min-h-screen ${s.bg} text-slate-700 font-sans pb-40 transition-colors duration-500 overflow-x-hidden selection:bg-rose-100`}>
      <header className={`${s.header} pt-10 pb-16 px-6 text-center relative overflow-hidden transition-all duration-500`}>
        ...
        <h1 className="text-2xl font-black tracking-widest relative z-10 flex items-center justify-center gap-2 cursor-pointer" onClick={() => { setView('main'); setSearchTerm(''); }}><Music size={28} /> おんがく手帳</h1>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-xs rounded-[2.5rem] shadow-2xl p-6 overflow-y-auto max-h-[90vh]">
            ...
            <div className="space-y-4">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">メニュー</p>
              ...
              <div className="pt-2">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 ml-1">テーマ</p>
        ...
        {view === 'main' ? (
          <>
            ...
            <div className="flex gap-2 mb-6">
              ...
              <button onClick={openCamera} className={`bg-white p-4 ${s.button} ${s.accentText} shadow-xl active:scale-90 transition-all`}><Camera size={24} /></button>
              <button onClick={() => { setSelectedTerm(INITIAL_TERMS[Math.floor(Math.random()*INITIAL_TERMS.length)]); setAiAnalysis(null); }} className={`bg-white p-4 ${s.button} ${s.accentText} shadow-xl active:scale-90 transition-all`}><Shuffle size={24} /></button>
            </div>

            {/* Daily Term Card */}
            {!searchTerm && (
              <article onClick={() => { setSelectedTerm(termOfDay); setAiAnalysis(null); }} className={`${theme === 'kawaii' ? 'bg-gradient-to-br from-rose-50 to-orange-50' : 'bg-slate-800 text-white'} p-6 ${s.card} shadow-lg mb-8 relative overflow-hidden group cursor-pointer active:scale-[0.98] transition-all animate-in zoom-in-95 duration-500`}>
            ...
            <div className="space-y-3">
              ...
              {filteredTerms.length === 0 && (
                <div className="text-center py-20 bg-white/40 rounded-[3rem] border-4 border-dashed border-rose-50 animate-pulse">
                  <Sparkles className="mx-auto text-rose-100 mb-2" size={48} />
                  <p className="text-rose-300 font-black tracking-widest uppercase text-xs">見つかりませんでした</p>
                </div>
              )}
            </div>
          ...
          <div className="bg-white rounded-[2.5rem] p-8 shadow-xl text-left relative overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
            <button onClick={() => setView('main')} className={`${s.accentText} font-bold text-sm mb-6 flex items-center gap-1 active:scale-95 transition-all`}>← 辞典にもどる</button>
            
            {view === 'contact' && (
              <div className="space-y-6">
                <h2 className="text-2xl font-black text-slate-800 flex items-center gap-2">Contact</h2>
                <div className="space-y-3">
                  <a href={`mailto:${contactEmail}`} className={`flex items-center justify-between p-5 ${theme === 'kawaii' ? 'bg-rose-50 text-rose-500' : 'bg-slate-50 text-slate-600'} border-2 border-transparent rounded-3xl font-black text-sm active:scale-95 transition-all shadow-sm`}>
                    <div className="flex items-center gap-3"><Mail size={20}/> メールを送る</div>
                    <ChevronRight size={18}/>
                  </a>
                  <a href={contactFormUrl} target="_blank" rel="noopener noreferrer" className={`flex items-center justify-between p-5 ${theme === 'kawaii' ? 'bg-orange-50 text-orange-500' : 'bg-indigo-50 text-indigo-600'} border-2 border-transparent rounded-3xl font-black text-sm active:scale-95 transition-all shadow-sm`}>
                    <div className="flex items-center gap-3"><ExternalLink size={20}/> 問い合わせフォーム</div>
                    <ChevronRight size={18}/>
                  </a>
                </div>
              </div>
            ...
            {view === 'privacy' && (
              <div className="space-y-4">
                ...
                <p className="text-xs text-slate-500 font-bold leading-loose">当サイトは広告配信にGoogle AdSenseを利用しています。詳細は <code className="bg-slate-100 px-1 rounded">/privacy.html</code> を参照してください。</p>
              </div>
            )}
      ...
      <div className="fixed bottom-6 right-6 flex flex-col items-end gap-3 z-[100]">
        {isMetronomeOpen && (
          <div className={`bg-white p-6 ${theme === 'kawaii' ? 'rounded-[2.5rem]' : 'rounded-2xl'} shadow-2xl border-2 ${theme === 'kawaii' ? 'border-rose-100' : 'border-slate-200'} w-72 animate-in slide-in-from-bottom-4 duration-300`}>
            <div className="flex justify-between items-center mb-2"><span className={`${s.accentText} font-black text-[10px] uppercase tracking-widest`}>Metronome</span><button onClick={() => { setIsMetronomeOpen(false); setIsPlaying(false); }} className="text-slate-300 hover:text-slate-500 active:scale-90"><X size={20}/></button></div>
            <div className="grid grid-cols-4 gap-1 mb-4">
              {[2, 3, 4, 6].map(num => (
                <button key={num} onClick={() => { setBeatsPerMeasure(num); beatRef.current = 0; setCurrentBeat(0); }} className={`py-1 rounded-xl text-[10px] font-black transition-all ${beatsPerMeasure === num ? s.accent + ' text-white shadow-md' : 'bg-slate-100 text-slate-400'}`}>{num === 6 ? '6/8' : `${num}拍子`}</button>
              ))}
            </div>
            <div className="text-center mb-6">
              <div className={`text-5xl font-black text-slate-800 mb-1 transition-all duration-75 ${isPlaying && currentBeat === 0 ? 'scale-110 ' + s.accentText : 'scale-100'}`}>{bpm}</div>
              <div className="flex justify-center gap-1.5 mb-2">
                {[...Array(beatsPerMeasure)].map((_, b) => (
                  <div key={b} className={`w-2.5 h-2.5 rounded-full transition-all ${isPlaying && currentBeat === b ? s.accent + ' scale-125 shadow-md' : 'bg-slate-100'}`} />
                ))}
              </div>
            </div>
            <div className="flex items-center gap-3 mb-6 px-2">
              <button onClick={() => setBpm(Math.max(40, bpm - 1))} className="p-2 bg-slate-100 text-slate-500 rounded-full active:scale-90"><Minus size={18}/></button>
              ...
              <button onClick={() => setBpm(Math.min(240, bpm + 1))} className="p-2 bg-slate-100 text-slate-500 rounded-full active:scale-90"><Plus size={18}/></button>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[130] flex items-center justify-center p-4 overflow-y-auto">
          <article className={`bg-white w-full max-w-sm ${theme === 'kawaii' ? 'rounded-[3.5rem]' : 'rounded-2xl'} shadow-2xl p-8 relative max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200 text-center`}>
            ...
            <div className="space-y-4 mb-8 text-left">
              <div className={`${theme === 'kawaii' ? 'bg-rose-50/50 rounded-[2.5rem] border-rose-100/50' : 'bg-slate-50 rounded-xl border-slate-200'} p-6 border shadow-inner text-center`}>
                <p className="text-slate-800 font-black text-xl mb-3 italic leading-relaxed">"{selectedTerm.meaning}"</p>
                <p className="text-slate-600 text-xs leading-relaxed px-2">{selectedTerm.detail}</p>
              </div>

              {/* AI Insight Section */}
              <div className="bg-gradient-to-br from-indigo-50 to-purple-50 p-4 rounded-3xl border border-indigo-100">
                <div className="flex justify-between items-center mb-2">
                  <p className="text-[10px] font-black text-indigo-600 flex items-center gap-1 uppercase tracking-widest"><Sparkles size={12}/> AI名曲検索</p>
                  {!aiAnalysis && !isAiLoading && <button onClick={() => getAiMusic(selectedTerm.term)} className="text-[10px] font-black bg-indigo-600 text-white px-3 py-1 rounded-lg shadow-sm active:scale-95 transition-all">検索</button>}
                </div>
                {isAiLoading && <div className="text-indigo-300 text-[10px] font-bold py-2 animate-pulse text-center">AI探索中...</div>}
                {aiAnalysis && <p className="text-xs text-slate-600 font-bold leading-relaxed animate-in fade-in duration-500">{aiAnalysis}</p>}
              </div>

              {/* Personal Memo Section */}
              <div className={`${theme === 'kawaii' ? 'bg-amber-50/40 border-amber-100/50 rounded-3xl' : 'bg-slate-50 border-slate-200 rounded-xl'} p-4 border shadow-sm`}>
                ...
                <textarea className="w-full bg-transparent border-none text-sm text-slate-600 focus:ring-0 h-16 outline-none resize-none placeholder:text-slate-300" placeholder="練習の気づきをメモ..." value={memos[selectedTerm.id] || ''} onChange={(e) => setMemos({...memos, [selectedTerm.id]: e.target.value})} />
              </div>
            </div>

            <div className="flex flex-col gap-2">
              <button onClick={() => { const n = new Set(mastered); if(n.has(selectedTerm.id)) n.delete(selectedTerm.id); else n.add(selectedTerm.id); setMastered(n); }} className={`w-full py-5 ${theme === 'kawaii' ? 'rounded-[2.2rem]' : 'rounded-xl'} font-black text-white ${mastered.has(selectedTerm.id) ? 'bg-green-400 shadow-green-100' : s.accent + ' shadow-rose-100'} shadow-lg active:scale-95 transition-all`}>
      ...
      {isCameraOpen && (
        <div className="fixed inset-0 bg-black z-[150] flex flex-col items-center">
          ...
          <div className={`w-full bg-white ${theme === 'kawaii' ? 'rounded-t-[40px]' : 'rounded-none'} p-8 flex flex-col items-center gap-4 shadow-2xl animate-in slide-in-from-bottom-full duration-300`}>
            ...
            <button onClick={captureAndScan} disabled={isScanning} className={`w-20 h-20 rounded-full border-4 ${theme === 'kawaii' ? 'border-rose-400' : 'border-indigo-600'} p-1 flex items-center justify-center active:scale-90 shadow-xl`}>
              {isScanning ? <Loader2 className={`animate-spin ${theme === 'kawaii' ? 'text-rose-400' : 'text-indigo-600'}`} size={32} /> : <div className={`w-full h-full ${theme === 'kawaii' ? 'bg-rose-400' : 'bg-indigo-600'} rounded-full`} />}
            </button>
      ...
      <div className={`fixed bottom-0 left-0 right-0 ${theme === 'kawaii' ? 'bg-white/70 text-rose-300' : 'bg-slate-900/80 text-slate-400'} backdrop-blur-md py-1.5 text-center pointer-events-none md:hidden border-t border-white/10 z-40`}>
        <p className="text-[8px] font-black tracking-[0.4em] uppercase">Database: {INITIAL_TERMS.length} Terms / v7.5</p>
      </div>
    </div>
  );
}
