  ...
  const getAiMusic = async (term) => {
    ...
    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({ contents: [{ parts: [{ text: `音楽用語「${term}」が象徴的な有名なクラシック曲を1つ挙げ、その理由を30文字程度で解説。` }] }] })
      });
    ...
    <div className={`min-h-screen ${s.bg} text-slate-700 font-sans pb-40 transition-colors duration-500 overflow-x-hidden selection:bg-rose-100`}>
      ...
      <main className="max-w-md mx-auto px-4 -mt-8 relative z-20">
        {view === 'main' ? (
          ...
          <div className="bg-white rounded-[2.5rem] p-8 shadow-xl text-left relative overflow-hidden animate-in slide-in-from-bottom-4">
            ...
            <div className="text-center mb-6">
              ...
              <div className="flex justify-center gap-1.5 mb-2">
                {[...Array(beatsPerMeasure)].map((_, b) => (
                  <div key={b} className={`w-2.5 h-2.5 rounded-full transition-all ${isPlaying && currentBeat === b ? s.accent + ' scale-125 shadow-md' : (beatsPerMeasure === 6 && (b === 0 || b === 3) ? 'bg-slate-300' : 'bg-slate-100')}`} />
                ))}
              </div>
            </div>
            ...
            <div className="space-y-4 mb-8 text-left">
              ...
              <div className="bg-gradient-to-br from-indigo-50 to-purple-50 p-4 rounded-3xl border border-indigo-100">
                <div className="flex justify-between items-center mb-2">
                  ...
                  {!aiAnalysis && !isAiLoading && <button onClick={() => getAiMusic(selectedTerm.term)} className="text-[10px] font-black bg-indigo-600 text-white px-3 py-1 rounded-lg">検索</button>}
                </div>
              ...
              <div className={`${theme === 'kawaii' ? 'bg-amber-50/40 border-amber-100/50 rounded-3xl' : 'bg-slate-50 border-slate-200 rounded-xl'} p-4 border shadow-sm`}>
                ...
                <textarea className="w-full bg-transparent border-none text-sm text-slate-600 h-16 outline-none resize-none" placeholder="練習の気づきをメモ..." value={memos[selectedTerm.id] || ''} onChange={(e) => setMemos({...memos, [selectedTerm.id]: e.target.value})} />
              </div>
            </div>
